...
### Chart
![Performance Chart](/backtests/legacy/adaptive-quant/morpho_crossover_backtest_20260501.png)
...
3. **`backtests/legacy/adaptive-quant/backtest_adaptive_simple.R`** - Standalone backtest script (moved here during repo cleanup; run from repo root, see the script header for path notes)
4. **`eth_strategy_performance.png`** - Performance visualization (regenerated by the script above, not committed)
