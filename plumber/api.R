...
if (exists("ofile") && !is.null(ofile <- sys.frame(1)$ofile)) {
  ...
  # Fallback: assume we're running from plumber/ directory
  script_dir = normalizePath(".")
}

# Navigate to repo root
if (basename(script_dir) == "plumber") {
...
# Source files from new structure - order matters! db.R before graphQL.R
sources(c("plumber/db.R","plumber/messaging.R","plumber/encryption.R","plumber/getGasBalances.R","plumber/helpers/graphQL.R","plumber/helpers/apiHelpers.R","plumber/executeTrades.R","tradebot/tradebot.R"))
