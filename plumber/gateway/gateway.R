...
if (basename(script_dir) == "gateway") {
  wd = dirname(dirname(script_dir))
} else if (basename(script_dir) == "plumber") {
  wd = dirname(script_dir)
} else if (file.exists(file.path(script_dir, "plumber", "gateway", "gateway.R"))) {
  wd = script_dir
} else {
  wd = dirname(script_dir)
}
...
# Source files from new structure
source(paste0(wd,"plumber/helpers/apiHelpers.R"))
source(paste0(wd,"plumber/messaging.R"))
source(paste0(wd,"plumber/reporting.R"))   # <- contains send_request_report
