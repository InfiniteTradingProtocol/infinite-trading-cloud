# ============================================================
# btc-crossoverv3-backtest.R
# BTC-USD EMA Crossover v3 Strategy Backtest
#
# Strategy Parameters:
...
#
# Run: Rscript backtests/btc-crossoverv3-backtest.R
# ============================================================

source(file.path(dirname(normalizePath(
  ifelse(interactive(), "backtests/btc-crossoverv3-backtest.R",
         commandArgs(trailingOnly = FALSE) |>
           (\(a) a[startsWith(a, "--file=")])() |>
           (\(a) if (length(a)) sub("--file=", "", a) else "backtests/btc-crossoverv3-backtest.R")())
...
# (join daily SMA50 / RSI14 back to each 6h bar by date)
...
cat("\n══════════════════════════════════════════\n")
cat(" BTC crossOverV3 Strategy Parameters\n")
cat("══════════════════════════════════════════\n")
cat(sprintf("  Asset              : %s\n", PAIR))
...
out_path <- file.path(script_dir, "backtest_charts",
  sprintf("btc_crossoverv3_%s.png", format(Sys.Date(), "%Y%m%d")))
...
metrics <- build_chart(
  ...
  title         = "BTC-USD EMA Crossover v3 — Decaying Re-entry Reference  [Strategy = v3]",
  subtitle      = sprintf(
