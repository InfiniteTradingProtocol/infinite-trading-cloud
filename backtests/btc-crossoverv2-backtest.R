# ============================================================
# btc-crossoverv2-backtest.R
# BTC-USD EMA Crossover v2 Strategy Backtest
#
...
#
# Run: Rscript backtests/btc-crossoverv2-backtest.R
# ============================================================

source(file.path(dirname(normalizePath(
  ifelse(interactive(), "backtests/btc-crossoverv2-backtest.R",
         commandArgs(trailingOnly = FALSE) |>
           (\(a) a[startsWith(a, "--file=")])() |>
           (\(a) if (length(a)) sub("--file=", "", a) else "backtests/btc-crossoverv2-backtest.R")())
...
cat("\n══════════════════════════════════════════\n")
cat(" BTC crossOverV2 Strategy Parameters\n")
cat("══════════════════════════════════════════\n")
cat(sprintf("  Asset         : %s\n", PAIR))
...
out_path <- file.path(script_dir, "backtest_charts",
  sprintf("btc_crossoverv2_%s.png", format(Sys.Date(), "%Y%m%d")))
...
metrics <- build_chart(
  ...
  title         = "BTC-USD EMA Crossover v2 — Strategy Backtest",
  subtitle      = sprintf(
    "Period: %s to %s  |  60%% EMA threshold + SMA(50) + RSI>45 + 8%% trailing stop  |  Benchmarks: BTC & ETH",
