"""
rotation_btcstrat_v2.py
Enhanced version of rotation_btcstrat.py — SAME underlying BTCStrat
dual-mode signal logic (unchanged, via dual_mode_signal.py), with the
improved position management overlay from rotation_engine_v2.py (15%
trailing stop + 5-day switch deadband). Does not modify
rotation_btcstrat.py, dual_mode_signal.py, or rotation_engine.py.

Run: python3 backtests/rotation/rotation_btcstrat_v2.py
"""
import os
import sys
import numpy as np
...
report(
    name="BTCStrat (Dual-Mode) Rotation v2",
    out_name="rotation_btcstrat_v2.png",
    common=common, strat_ret=strat_ret, btc_ret=btc_ret, eth_ret=eth_ret,
    holdings=holdings, stop_events=stop_events,
    base_signal_desc="Trend: 50% EMA+SMA50+RSI>40 | Accum: >=30% dip / RSI<40 / >=5% bounce",
