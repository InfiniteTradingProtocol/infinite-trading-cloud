...
csv_path <- "../data/ETH_6H_2016-05-18_2026-04-15.csv"
...
if (file.exists(csv_path)) {
  ...
  start_date <- format(min(df$time), "%Y-%m-%d")
  end_date <- format(max(df$time), "%Y-%m-%d")
  csv_filename <- sprintf("../data/ETH_6H_%s_%s.csv", start_date, end_date)
...
# Plot setup
png("charts/trend_rider_performance.png", width = 1400, height = 900)
...
cat("✓ Chart saved to: charts/trend_rider_performance.png\n")
...
write.csv(results_df, "trend_rider_results.csv", row.names = FALSE)
cat("✓ Results saved to: trend_rider_results.csv\n")
...
write.csv(equity_df, "trend_rider_equity.csv", row.names = FALSE)
cat("✓ Equity curve saved to: trend_rider_equity.csv\n")
