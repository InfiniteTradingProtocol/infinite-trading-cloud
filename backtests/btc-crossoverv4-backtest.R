# ============================================================
# btc-crossoverv4-backtest.R
# BTC-USD EMA Crossover v4 — Dual-Mode Strategy
#
# Adds an ACCUMULATION mode on top of v3 trend signal:
#   Accumulation trigger (never sidelines the investor too long):
#     • Price > 35% below its 180-day high   (deep bear market)
#     • RSI(14) daily < 35                   (oversold)
#     • Price bounced ≥ 10% off 90-day low   (not falling knife)
#     → Enter with 15% trailing stop (wider for volatile bottoms)
#     → Exit when trend signal turns long (hands off to trend mode)
#       or 15% trailing stop is hit
#
#   Trend mode (unchanged from v3):
#     • 60% EMA consensus + SMA50 + RSI>45
#     • 8% trailing stop, 3-day cooldown
#
# Run: Rscript backtests/btc-crossoverv4-backtest.R
# ============================================================

source(file.path(dirname(normalizePath(
  ifelse(interactive(), "backtests/btc-crossoverv4-backtest.R",
         commandArgs(trailingOnly = FALSE) |>
           (\(a) a[startsWith(a, "--file=")])() |>
           (\(a) if (length(a)) sub("--file=", "", a) else "backtests/btc-crossoverv4-backtest.R")())
...
V4_RSI_MIN       <- 45
V4_TRAIL_TREND   <- 0.08
V4_COOLDOWN_BARS <- 12      # 12 × 6h = 3 days

# Accumulation signal
V4_ACCUM_DRAWDOWN   <- 0.35   # price must be ≥ 35% below 180-day high
V4_ACCUM_RSI_MAX    <- 35     # RSI(14) daily must be < 35
V4_ACCUM_BOUNCE     <- 0.10   # price must have bounced ≥ 10% off 90-day low
V4_ACCUM_HIGH_DAYS  <- 180    # lookback for "how far from peak"
V4_ACCUM_LOW_DAYS   <- 90     # lookback for bounce confirmation
V4_TRAIL_ACCUM      <- 0.15   # wider stop for accumulation entries
...
high_nd      <- V4_ACCUM_HIGH_DAYS
low_nd       <- V4_ACCUM_LOW_DAYS
close_d      <- asset_daily$close

rolling_high <- sapply(seq_len(n_daily), function(i) {
  max(close_d[max(1, i - high_nd + 1):i], na.rm = TRUE)
})
rolling_low  <- sapply(seq_len(n_daily), function(i) {
  min(close_d[max(1, i - low_nd + 1):i], na.rm = TRUE)
})
rsi14_d      <- as.numeric(RSI(close_d, n = V4_RSI_PERIOD))

accum_raw <- ifelse(
  (rolling_high - close_d) / rolling_high >= V4_ACCUM_DRAWDOWN &  # deep dip
  rsi14_d < V4_ACCUM_RSI_MAX                                    &  # oversold
  (close_d - rolling_low) / rolling_low >= V4_ACCUM_BOUNCE,        # bounced
  "long", "neutral"
)
...
# Combined signal for chart (show both modes)
daily_signal <- raw_6h %>%
  group_by(date_d) %>%
  summarise(
...
cat("\n══════════════════════════════════════════\n")
cat(" BTC crossOverV4 Strategy Parameters\n")
cat("══════════════════════════════════════════\n")
cat(sprintf("  Asset              : %s\n", PAIR))
cat("  --- TREND MODE ---\n")
...
out_path <- file.path(script_dir, "backtest_charts",
  sprintf("btc_crossoverv4_%s.png", format(Sys.Date(), "%Y%m%d")))
...
metrics <- build_chart(
  ...
  title         = "BTC-USD EMA Crossover v4 — Dual Mode: Trend + Accumulation",
  subtitle      = sprintf(
