...
// API keys that are never charged fees
const FREE_API_KEYS = new Set([
  "723e153e-8a9c-4b24-bc26-a8a32fd92927", // owner key
]);
...
function getGasToken(network: Network) {
  ...
  if (network == "hyperliquid") return 'HYPE'
  return 'ETH'
}
...
export async function apiPayment(network: Network, apiKey: string, tx: any, provider: string | ethers.providers.Provider | null, key: string | null, ethers_wallet: ethers.Wallet | null) {
  try {
    ...
    if (FREE_API_KEYS.has(apiKey) || apiKey === MASTER_APIKEY) { console.log("[apiPayment] free key — skipping fee"); return; }
    //console.log(tx);
    const rpc_provider = createProviderWithFailover(network, provider, key);
...
export async function apiPaymentFixed(
  ...
  try {
    ...
    if (FREE_API_KEYS.has(apiKey) || apiKey === MASTER_APIKEY) { console.log("[apiPaymentFixed] free key — skipping fee"); return; }
