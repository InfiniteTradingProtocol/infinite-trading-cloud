 ...
 *  - On no candles found (valid params, but table doesn't exist or query
 *    returns 0 rows): returns `{}` — CONFIRMED via live curl against both R
 *    (https://api.infinitetrading.io) and this port (2026-09-06), exact
 *    match for both the empty-table case and a real coinbase_BTC-USD_6h
 *    query (3 candles, byte-for-byte identical JSON).
 */
