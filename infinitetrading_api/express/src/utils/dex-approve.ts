...
/**
 * Fetch appropriate gas options for the approve transaction.
 * Polygon requires a minimum 25 gwei tip cap; other networks use provider getFeeData.
 */
async function getApproveGasOptions(network: Network): Promise<Record<string, any>> {
    try {
        if (network === Network.POLYGON) {
            const { data } = await axios.get("https://gasstation.polygon.technology/v2");
            const toWei = (gwei: number) =>
                ethers.utils.parseUnits(Math.max(gwei * 1.2, 30).toFixed(9), "gwei").toString();
            return {
                maxPriorityFeePerGas: toWei(data.fast.maxPriorityFee ?? 30),
                maxFeePerGas: toWei(data.fast.maxFee ?? 60),
                type: 2,
            };
        }
        // For all other networks, let the SDK / provider resolve gas naturally
        return {};
    } catch (e) {
        console.warn("[approveGas] Gas fetch failed, using SDK defaults:", (e as Error).message);
        if (network === Network.POLYGON) {
            // Hard minimum fallback so the tx doesn't revert on Polygon
            return {
...
export async function approveIfNeeded(
    ...
    try {
        ...
        const gasOptions = await getApproveGasOptions(network);
        const tx = await pool.approve(dex, assetAddress, MAX_ALLOWANCE, gasOptions);
