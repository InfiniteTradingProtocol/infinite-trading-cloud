...
#
# Packages will be automatically installed if not available
#
...
pool = "0xdf6aee03da0aabe63105780233b8c03aa50067d4"    #Address of your pool (smart contract address of your pool).
pair = "OP-USDC"       #The pair to trade.
...
while (1) {
  tryCatch({
  candles <- get_candles_with_retry(pair = "OP-USD", numcandles = 300, timeframe = "6h")
  print(candles)
  ...
  else if (PROBABILITY_UP < 0.40 && last_side == "long") { side = "neutral" }
  else { side = "hold" } 
