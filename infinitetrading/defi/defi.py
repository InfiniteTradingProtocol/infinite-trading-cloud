...
sys.path.append(f"{parent_dir}/infinitetrading/db/")
import credentials
...
def compositions(protocol,network,pool):
    if protocol == "defund":
        composition = defundComposition(pool,network)
    elif protocol == "dhedge":
        composition = dhedgeComposition(pool,network)
    elif protocol == "factordao":
        composition = factordaoComposition(pool,network)
    elif protocol == "enzyme":
        composition = enzymeComposition(pool,network)
    else:
        return 0
    return(composition)

def dhedgeComposition(pool,network):
    url = f"http://ec2-3-135-99-211.us-east-2.compute.amazonaws.com:8000/poolComposition?network={network}&pool={pool}"
    ...
    push_composition(pool,df)
    return df

def push_composition(pool,df):
    #Connect to the database
    cnx = credentials.cnx
    # Create a cursor object
    cursor = cnx.cursor()
    pool = f"`{pool}`"
    cursor.execute(f"CREATE TABLE IF NOT EXISTS {pool} (asset VARCHAR(255) PRIMARY KEY, isDeposit BOOLEAN, assetPair VARCHAR(255), symbol VARCHAR(255), amount FLOAT, price FLOAT)")
    cnx.commit()
    cursor.execute(f"UPDATE {pool} SET amount = 0")
    # Insert DataFrame to Table
    for i,row in df.iterrows():
       sql = f"INSERT INTO {pool} (asset, isDeposit, assetPair, symbol, amount, price) VALUES (%s,%s,%s,%s,%s,%s) ON DUPLICATE KEY UPDATE isDeposit=VALUES(isDeposit), assetPair=VALUES(assetPair), symbol=VALUES(symbol), amount=VALUES(amount), price=VALUES(price)"
       cursor.execute(sql, tuple(row))
    # the connection is not autocommitted by default, so we must commit to save our changes
    cnx.commit()

def get_composition(pool):
    cnx = credentials.cnx
    cursor = cnx.cursor()
    pool = f"`{pool}`"
    query = f"SELECT * FROM {pool}"
    cursor.execute(query)
    rows = cursor.fetchall()
    column_names = ['asset', 'isDeposit', 'assetPair', 'symbol', 'amount', 'price']
    data = []
    for row in rows:
        data.append(row)
...
def get_balance(asset, composition, bignumber=False):
    asset_row = composition[composition.iloc[:, 2] == asset].index
    asset_balance = 0
    if not bignumber:
        if len(asset_row) > 0:
            asset_balance = pd.to_numeric(composition.iloc[asset_row, 4], errors='coerce').item()
    else:
        if len(asset_row) > 0:
            asset_balance = pd.to_numeric(composition.iloc[asset_row, 4], errors='coerce').item()
...
def get_price(asset, composition, bignumber=False):
    asset_row = composition[composition.iloc[:, 2] == asset].index
    asset_price = 0
    if len(asset_row) > 0:
        asset_price = pd.to_numeric(composition.iloc[asset_row, 5], errors='coerce').item()
    return asset_price

def get_usdc_balance(composition):
    balance = get_balance(asset="USDC",composition=composition)
    return balance

def get_allocation(asset, pool=None, composition=None, network="polygon", protocol="dhedge",verbose=False):
    if composition is None:
        composition = compositions(pool, network, protocol)
    if composition.shape[1] is None:
        raise ValueError(f"Error: failed to load the pool composition for rebalancing this pool: {pool} / network: {network}")
    ...
    balance = 0
    for i in range(n):
        assets[i] = composition.iloc[i,2]
        balances[i] = get_balance(assets[i], composition)
        prices[i] = get_price(assets[i],composition)
        if asset == assets[i]:
            balance = balances[i]
            price = prices[i]
    total_value = np.sum(balances * prices)
    if verbose:
        print(f"assets {assets} / balances {balances} / prices {prices}")
    return (balance * price) / total_value

def get_minmax_from_composition(composition, which,allowed_assets=None):
    n=composition.shape[0]
    balances = np.zeros(n,dtype="float")
    prices = np.zeros(n,dtype="float")
    assets = [""]*n
    ...
    for i in range(n):
        assets[i] = composition.iloc[i,2]
        if add_assets:
            allowed_assets.append(assets[i])
        balances[i] = get_balance(assets[i], composition)
        prices[i] = get_price(assets[i],composition)
        total_usd = balances[i]*prices[i]
        if which == "max":
            if usd <= total_usd and assets[i] in allowed_assets:
                usd = total_usd
                asset = assets[i]
        if which == "min" and assets[i] in allowed_assets:
            if usd >= total_usd:
                usd = total_usd
                asset = assets[i]
    total_value = np.sum(balances * prices)
    return {"asset": asset, "allocation": total_usd/total_value, "USDvalue": usd }
...
def toros(coin):
    coin = coin.lower()
    if coin == "ethbear2x" or coin == "btcbear2x" or coin == "maticbear1x" or coin == "ethbear1x" or coin == "btcbear1x" or coin == "usdmny" or coin == "ethy" or coin == "maticbull2x" or coin == "ethbull3x" or coin == "btcbull3x":
        return True
    return False

def trade(ep="local", from_coin=None, to_coin=None, platform="ONEINCH", network=None, type="swap", share=100, slippage=1, pool=None, amount=None, protocol="dhedge",channel="#pools-trading"):
    trade_ep = ""
    #I need to create "customers" table for endpoints, ports or keys to be able to manage multiple accounts
    if ep == "local":
        if protocol == "dhedge":
            trade_ep = 'http://localhost:8000/trade?from='
        elif protocol == "defund":
            trade_ep = 'http://localhost:3000/trade?from='

    from_c = coin_to_contract(from_coin, network=network)
    to_c = coin_to_contract(to_coin, network=network)

    if amount is None:
        ep = f"{trade_ep}{from_c}&to={to_c}&share={share}&slippage={slippage}&pool={pool}"
    else:
        decimals_places = decimals(from_coin)
        amount = int(amount * 10 ** decimals_places)
        ep = f"{trade_ep}{from_c}&to={to_c}&amount={amount}&slippage={slippage}&pool={pool}"

    if protocol == "dhedge":
        if network and network != "polygon":
            ep = f"{ep}&network={network}"
        if platform and platform != "ONEINCH":
            ep = f"{ep}&platform={platform}"
        if platform == "uniswapV3":
            fee = uniswap_fees(coin1=from_coin, coin2=to_coin, network=network)
            ep = f"{ep}&feeAmount={fee}"
        pool_url = f"https://app.dhedge.org/pool/{pool}"
    elif protocol == "defund":
        ep = f"{ep}&network={network}"
        if network == "polygon":
            chainid = 137
        pool_url = f"https://defund.io/v1.5.0/app/funds/{chainid}/{pool}"

    msg = f"Sending trade instruction: from: {from_coin} ({from_c}) to: {to_coin} ({to_c}) / network: {network} / pool: {pool_url} / end point: {ep}"
    send_message(msg, channel=channel, platform="discord")
    print(msg)
    ...
    if res is not None:
        if "status" in res and "msg" in res:
            send_message(f"[Trade transaction response] status: {res['status']} tx hash: {res['msg']}",channel=channel,platform="discord")


#pool="0x2a09812dbba5dd84a4a63038b1cc65f5f7e9229d"
#network = "polygon"
#asset="MATICBEAR1X"
#res=dhedgeComposition(pool,network)
#print(res)
#list1 = get_minmax_from_composition(composition=res, which="max")
#list2 = get_minmax_from_composition(composition=res, which="min")
#print(list1)
#print(list1['asset'])
#print(list2)
#print(list2['asset'])

#balance = get_balance(asset,composition=res)
#price = get_price(asset,composition=res)
#allocation = get_allocation(asset,pool=pool,composition=res)
#composition_from_db = get_composition(pool)
#print("MATICBEAR1X Balance:")
#print(balance)
#print("MATICBEAR1X Price:")
#print(price)
#print("MATICBEAR1X Allocation:")
#print(allocation)
#print("Compositions from the DB")
#print(composition_from_db)
