...
#        → exit Fluid/Compound, borrow USDC to HF=1.5, swap USDC→GHO in $1000
#          chunks, lend GHO on AAVE
#   2. Fluid fUSDC / Compound V3 USDC — existing logic (spread > 0.5pp open)
#
...
# Returns the vault's exact USDC variable debt in wei (integer, 6 decimals)
# Reads variableDebtUSDC.balanceOf() directly — no USD oracle conversion
get_usdc_debt_wei <- function() {
  ...
  # resp$data = "50.123456" (USDC, 6 decimal places, already formatted)
...
# Convert USD amount to GHO wei string (18 decimals, floored)
...
ensure_gho_approved_aave <- function() {
  ...
  # /approve reads asset from req.body (JSON), not query params
...
# Swap USDC → GHO in $MAX_SWAP_USD chunks (blocks until done)
# total_usd: approximate USD to swap (USDC 1:1 with USD)
...
get_gho_balance_usd <- function() {
  ...
  if (is.null(resp$data)) return(NULL)
  as.numeric(resp$data$balance)   # API returns token amount already formatted
}
...
# Swap GHO → USDC in $MAX_SWAP_USD chunks (blocks until done).
# Returns TRUE if all GHO was swapped, FALSE if any chunk failed.
# total_usd: USD value to swap; if NULL/NA uses actual vault GHO balance.
swap_gho_to_usdc_loop <- function(total_usd = NULL) {
  # Use real wallet balance when caller passes NULL or when balance is available
...
# ── Yield harvest ─────────────────────────────────────────────────────────────
# Called BEFORE any repay. If the vault holds more USDC than the on-chain debt
# (i.e. yield earned > borrow cost), swap the surplus to wstETH and lend it
# to AAVE as additional collateral — compounding the ETH yield.
# Threshold: only harvest if surplus > $5 to avoid wasting gas on dust.
MIN_HARVEST_USD <- 5.0

harvest_yield_surplus <- function() {
  # Get exact on-chain USDC debt (variableDebtToken balance)
  ...
  # Get vault's raw USDC balance (sitting idle after platform withdrawal)
  ...
  usdc_balance_usd <- as.numeric(bal_resp$data$balance)  # 6-dec token → USD 1:1
  ...
  # lend_eth_collateral() already handles wstETH/weETH/cbETH/WETH
  lend_eth_collateral()
...
# - Idle USDC (no active protocol): repay debt; if debt clears, swap residual to wstETH collateral
sweep_idle_assets <- function() {
  current_platform <- read_active_platform()

  # 1. Swap any GHO sitting in the vault (not currently lent)
  if (current_platform != "gho") {
    idle_gho <- get_gho_balance_usd()
    if (!is.null(idle_gho) && idle_gho > MIN_BORROW) {
      cat(sprintf("  \U0001f9f9 Idle GHO $%.4f in vault \u2014 swapping to USDC\n", idle_gho))
  ...
  if (current_debt > MIN_BORROW) {
    cat(sprintf("  \U0001f9f9 $%.4f idle USDC, $%.4f debt \u2014 repaying\n", idle_usdc, current_debt))
    repay_usdc_all()
    Sys.sleep(15)
  ...
  if (idle_usdc > MIN_BORROW) {
    if (isTRUE(cached_profitable)) {
      # Yield spread still open — deposit to Fluid/Compound rather than converting to collateral
      plat <- cached_best_platform %||% "fluid"
      cat(sprintf("  \U0001f9f9 $%.4f USDC idle, %s profitable \u2014 depositing\n", idle_usdc, toupper(plat)))
      if (plat == "fluid") {
      ...
      cat(sprintf("  \U0001f9f9 $%.4f USDC with no debt \u2014 converting to wstETH collateral\n", idle_usdc))
      r <- tryCatch(local_GET("trade", list(
        network  = NETWORK, pool = VAULT, platform = "odos",
        from = USDC, to = WSTETH, amount = to_usdc_wei(idle_usdc), slippage = 1, apiKey = apiKey
      )), error = function(e) list(status = "fail", msg = e$message))
      if (!is.null(r$status) && r$status == "success") {
        Sys.sleep(20)
        lend_eth_collateral()
      } else {
        cat(sprintf("  \u26a0\ufe0f  USDC\u2192wstETH sweep failed: %s\n", r$msg %||% r$message %||% "unknown"))
      }
    }
  }
}
...
cached_spread        <- 0
gho_profitable       <- FALSE   # cached GHO profitability (updated each APY check)
gho_spread           <- 0       # cached GHO spread (updated each APY check)
  ...
  tryCatch({
    ...
    do_withdraw <- function() {
      if (active_platform == "fluid") {
        ...
        # Unlend GHO first; caller is responsible for swapping GHO → USDC
        unlend_gho()
      } else {
    ...
    if (secs_since_apy >= APY_INTERVAL_SECS) {
      ...
      # GHO spread (absolute pp above USDC borrow APY)
      ...
      # Fluid/Compound spread (existing logic)
      ...
      if (gho_profitable) {
        ...
        if (active_platform != "gho") {
          ...
          # Exit any Fluid/Compound position first
          ...
          # Borrow USDC to reach TARGET_HF (only if needed)
          ...
          # Approve GHO for AAVE (once, idempotent)
          ensure_gho_approved_aave()

          # Swap all available USDC → GHO in $MAX_SWAP_USD chunks
          # Use tgt_debt as the total amount (= what we just borrowed)
          ...
          # Lend all GHO to AAVE
          lend_gho()
          write_active_platform("gho")
          active_platform <- "gho"

        } else {
          # Already holding GHO — check if we can borrow more (HF well above threshold)
        ...
        unlend_gho()
        Sys.sleep(30)
        # Use actual GHO wallet balance (not stale USDC debt) to handle yield accrual
        swap_ok <- swap_gho_to_usdc_loop(NULL)
        if (!swap_ok) {
          cat("  ⚠️  GHO→USDC swap failed — keeping state as 'gho', will retry next cycle\n")
        } else {
          Sys.sleep(30)
          harvest_yield_surplus()      # swap any earned surplus to wstETH collateral
          Sys.sleep(10)
          repay_usdc_all()
          Sys.sleep(10)
          clear_active_platform()
        ...
        # ── 3b. GHO not active/profitable: Fluid / Compound logic (unchanged) ──
        if (cached_profitable) {
          ...
          cat(sprintf("  ❌ Fluid/Compound spread %.4f%% below hold threshold (%.2f%%) → exiting\n",
          ...
          if (debt > MIN_BORROW) {
            ...
            clear_active_platform()
            active_platform <- "none"
            # Immediately convert any residual USDC to wstETH collateral
            Sys.sleep(5)
            sweep_idle_assets()
          }
        }
    ...
    if (active_platform != "none" && debt > MIN_BORROW) {
      if (hf < HF_LOW) {
        ...
        if (active_platform == "gho") {
          # GHO emergency: unlend GHO, swap back to USDC, then repay
          cat("  🔴 GHO emergency unwind\n")
          unlend_gho()
          Sys.sleep(30)
          # Use actual GHO wallet balance (handles yield accrual & depeg)
          swap_gho_to_usdc_loop(NULL)
          Sys.sleep(30)
        } else {
          do_withdraw()
        ...
        # Before repaying: harvest any yield surplus (swap to wstETH collateral)
        harvest_yield_surplus()
        Sys.sleep(10)

        # Fetch exact on-chain USDC debt (variableDebtToken.balanceOf) — avoids
        # USD oracle rounding that causes ERC20: transfer amount exceeds allowance
        ...
        if (!is.null(actual_debt_wei)) {
          ...
          if (!is.null(aave_fresh$data)) {
            ...
            fresh_tgt_debt_wei <- floor(tgt_debt * 1e6)  # fallback to stale
          }
        ...
        # Re-enter if spread is still profitable after repay
        ...
        tryCatch({
          ...
          if (hf_after < HF_LOW && can_send_alert(hf_alert)) {
            send_resend_email(
              subject   = sprintf("\U0001f6a8 ETH Vault HF Critical: %.4f", hf_after),
              html_body = sprintf(paste0(
                "<h2>&#128680; Health Factor Still Critical</h2>",
                "<p><strong>Strategy:</strong> ETH Yield Vault</p>",
