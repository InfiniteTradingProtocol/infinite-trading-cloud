...
required_packages = c("httr", "jsonlite", "lubridate", "TTR", "quantmod", "DBI", "RMariaDB", "dotenv", "pool")
...
require(DBI); require(RMariaDB); require(dotenv); require(httr); require(pool)
...
if (!exists("wd")) {
  ...
  # Navigate up from strategies/strategies/ to repo root
...
pool_file = paste0(wd, "strategies/strategies/db_pool.R")
if (file.exists(pool_file)) {
  tryCatch({
    source(pool_file)
  }, error = function(e) {
    warning(paste("Failed to initialize connection pool:", e$message))
  })
}
...
db_con = function(db=db_schema, use_pool=TRUE) {
        # Use connection pool if available and requested
        if (use_pool && exists("db_pool", envir = .GlobalEnv)) {
                if (db != "infinitetrading") {
                        # Different database requested, use direct connection
                        db_credentials = c(); db_credentials$user = db_user; db_credentials$ip = Sys.getenv("db_ip"); db_credentials$password = db_password; db_credentials$port = 3306
...
get_candles_from_mysql <- function(pair, timeframe) {
  ...
  on.exit(dbDisconnect(con), add = TRUE)  # CRITICAL FIX: Close connection
...
# Local MySQL functions for candles (ephemeral data)
local_candles_con <- function() {
  ...
  con <- dbConnect(RMariaDB::MariaDB(), 
                   ...
                   dbname = ifelse(Sys.getenv("db_name_local") != "", Sys.getenv("db_name_local"), "infinitetrading"), 
                   host = "127.0.0.1", 
                   port = 3306)
  return(con)
}

get_candles_from_local <- function(pair, timeframe) {
  con <- local_candles_con()
  ...
  raw <- tryCatch({
    ...
    message(sprintf("[LOCAL CANDLES] Error fetching from local MySQL: %s", e$message))
    NULL
  })
