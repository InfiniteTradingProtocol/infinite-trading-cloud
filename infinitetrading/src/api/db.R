...
setSide <- function(protocol, pool, network, pair, side, threshold, max_usd, share, platform, slippage,lending=FALSE) {
	response <- tryCatch({
  		...
  		res = write_table(schema="infinitetrading", table = table_name,names = names,types=types,values=values,primary_key="pool")
		cat("DEBUG setSide: write_table returned ", res, " (type:", class(res), ")\n", sep="")
		if (res == 1L) { list(status="success",status_code=200,message="Sides submitted succesfully") }
...
getBots <- function(manager, protocol = "dhedge") {
  ...
  all_tables <- tryCatch(dbListTables(con), error = function(e) character(0))
  
  # Build wallet list for IN clause
  wallet_list <- paste0("('", paste(gsub("'", "''", gasWallets$wallet), collapse = "','"), "')")
  
  # OPTIMIZED QUERY 1: Get all pools for all wallets across all networks in one query
  pools_union_parts <- c()
  for (network in networks) {
    table_gas <- paste(network, protocol, "gas_wallets", sep = "_")
    if (table_gas %in% all_tables) {
      pools_union_parts <- c(pools_union_parts, sprintf(
        "SELECT '%s' AS network, pool, wallet FROM %s WHERE wallet IN %s",
        network, table_gas, wallet_list
      ))
    }
  }
  
  if (length(pools_union_parts) == 0) {
    return(list(status = "success", status_code = 200, bots = list()))
  }
  
  pools_query <- paste(pools_union_parts, collapse = " UNION ALL ")
  all_pools <- tryCatch({
    dbGetQuery(con, pools_query)
  }, error = function(e) {
    cat("Error fetching pools:", e$message, "\n")
    return(data.frame())
  })
  ...
  # OPTIMIZED QUERY 2: Get all sides for all pools across all networks in one query
  sides_union_parts <- c()
  for (network in networks) {
    table_sides <- paste(network, protocol, "sides", sep = "_")
    if (table_sides %in% all_tables) {
      # Get unique pools for this network
      network_pools <- unique(all_pools[all_pools$network == network, "pool"])
      if (length(network_pools) > 0) {
        pool_list <- paste0("('", paste(gsub("'", "''", network_pools), collapse = "','"), "')")
        sides_union_parts <- c(sides_union_parts, sprintf(
          "SELECT '%s' AS network, pool, pair, side, threshold, max_usd, share, platform, slippage FROM %s WHERE pool IN %s",
          network, table_sides, pool_list
        ))
      }
    }
  }
  
  if (length(sides_union_parts) == 0) {
    return(list(status = "success", status_code = 200, bots = list()))
  }
  
  sides_query <- paste(sides_union_parts, collapse = " UNION ALL ")
  all_sides <- tryCatch({
    dbGetQuery(con, sides_query)
  }, error = function(e) {
    cat("Error fetching sides:", e$message, "\n")
    return(data.frame())
  })
  ...
  if (nrow(all_sides) > 0) {
    for (i in 1:nrow(all_sides)) {
      side_row <- all_sides[i,]
      # Find matching pool to get wallet
      pool_match <- all_pools[all_pools$network == side_row$network & all_pools$pool == side_row$pool,]
      if (nrow(pool_match) > 0) {
        pool_data <- list(
          pool = side_row$pool,
          gasWallet = pool_match$wallet[1],
          network = side_row$network,
          pair = side_row$pair,
          side = side_row$side,
          threshold = side_row$threshold,
          max_usd = side_row$max_usd,
          share = side_row$share,
          platform = side_row$platform,
          slippage = side_row$slippage
        )
...
getAPIKey = function(network,protocol,pool) {
        apiKey = tryCatch({
                ...
                return(secure_decrypt(res$encrypted_api_key[1]))
        }, 
        error = function(e) { 
                cat("Error retrieving API key: ",e$message,"\n")
                NULL
        })
        return(apiKey)
}
...
getWalletPools <- function(protocol, network, wallet) {
  ...
  result <- tryCatch({
    ...
    if (length(union_parts) == 0) {
      return(list(status = "success", status_code = 200, message = data.frame()))
    }
