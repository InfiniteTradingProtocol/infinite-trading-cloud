...
source(paste0(wd,'tradebot/tradebot.R'))
require(lubridate)
	...
	while (1) {
		...
		for (i in 1:n) {
			...
			if (side == "long") { 
				...
				if (again) { tradebot(pools[i],pair,share=100,slippage=2,threshold=0.5,side = "sell",price=price,ep=ep[i],network=networks[i],protocol=protocol,manager=manager) }
			}
			else if (side == "short") {
				...
				if (again) { tradebot(pools[i],pair,share=100,slippage=0.5,threshold=0.5,side = "buy",price=price,ep=ep[i],network=networks[i],protocol=protocol,manager=manager) }
			}
