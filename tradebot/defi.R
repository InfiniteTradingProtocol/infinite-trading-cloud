...
# wd should already be set by parent script
# If not set, calculate it
if (!exists("wd")) {
  wd = dirname(dirname(normalizePath(".")))
  wd = paste0(wd, "/")
}
...
require(dotenv)
tryCatch({
  if (file.exists(paste0(wd, ".env"))) {
    load_dot_env(paste0(wd, ".env"))
  }
}, error=function(e) {})

# Source dependencies (these will be sourced from parent in most cases)
# source(paste0(wd,"db.R")); source(paste0(wd,"slack.R")); source(paste0(wd,"telegram.R"))

# Load coins.csv from repo root
coins_csv_path = paste0(wd, "coins.csv")
if (file.exists(coins_csv_path)) {
  coins_data <- read.csv(coins_csv_path, colClasses = c("character", "character", "character"))
} else {
  cat(paste0("Warning: coins.csv not found at ", coins_csv_path, "\n"))
